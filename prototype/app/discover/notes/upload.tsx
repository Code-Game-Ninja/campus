...
export default function UploadNotes() {
  ...
  const submit = async () => {
    ...
    try {
      if (pendingCompletion) {
        ...
        await queryClient.invalidateQueries({ queryKey: apiQueryKey('resources', 'servable') });
        setSubmitted(true);
        toast({ type: 'success', message: 'Material uploaded and available in Notes.' });
        return;
      }
  ...
  if (submitted) {
    return <Screen><TopBar title="Share material" subtitle="Upload received" left={<IconButton icon="close" label="Close" onPress={() => router.replace('/discover/notes')} />} /><StateView icon="cloud-done-outline" title="Upload complete" detail="Your material is now available in Notes for students at your campus." /><View style={{ gap: 10 }}><Button label="Back to notes" icon="arrow-back" onPress={() => router.replace('/discover/notes')} /><Button variant="ghost" label="Upload another" icon="add-circle-outline" onPress={reset} /></View></Screen>;
  }
