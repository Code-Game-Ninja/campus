...
export default function NoteDetail() {
  ...
  const query = useApiQuery<ApiResource>(apiQueryKey('resource', id), `/resources/${id}`, {}, { enabled: Boolean(id) });
  const resource = query.data ? mapResource(query.data) : undefined;
  ...
  if (query.isError) return <Screen><StateView icon="cloud-offline" tone="danger" title="Resource unavailable" detail={query.error.message} action="Go back" onAction={() => goBackOrReplace('/discover/notes')} /></Screen>;
  if (!resource) return <Screen><StateView icon="document-outline" title="Resource not available" detail="It may be removed or outside your campus scope." action="Go back" onAction={() => goBackOrReplace('/discover/notes')} /></Screen>;
