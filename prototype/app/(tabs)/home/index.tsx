...
export default function Home() {
  ...
  const posts = useMemo(() => (feed.data?.items ?? []).map((post) => mapPost(post, me.data?.campusName ?? 'Your campus')), [feed.data?.items, me.data?.campusName]);
  const displayName = profile.data?.displayName ?? 'Campus member';
  const initials = displayName.split(/\s+/).map((part) => part[0]).join('').slice(0, 2).toUpperCase();
  const refresh = async () => {
    await Promise.all([me.refetch(), profile.refetch(), feed.refetch()]);
  };

  return <Screen refreshing={me.isRefetching || profile.isRefetching || feed.isRefetching} onRefresh={refresh}>
    <View style={{ flexDirection: 'row', alignItems: 'center', paddingTop: 8, paddingBottom: 16 }}>
      <View style={{ flex: 1 }}><Text style={{ color: p.muted, fontSize: 12, fontWeight: '700' }}>GOOD MORNING</Text><Text style={{ color: p.ink, fontSize: 24, fontWeight: '900', marginTop: 2 }}>Hello, {displayName.split(' ')[0]}</Text></View>
      <IconButton icon="search" label="Search" onPress={() => router.push('/search')} />
      <View style={{ width: 8 }} />
      <Pressable accessibilityRole="button" accessibilityLabel="Messages" onPress={() => router.push('/chat')} style={({ pressed }) => ({ width: 44, height: 44, borderRadius: 22, backgroundColor: p.surface, borderWidth: 1, borderColor: p.line, alignItems: 'center', justifyContent: 'center', opacity: pressed ? 0.72 : 1 })}><Ionicons name="chatbubbles-outline" size={21} color={p.text} /></Pressable>
      <View style={{ width: 8 }} />
      <Pressable onPress={() => router.push('/(tabs)/profile')}><Avatar initials={initials || 'CS'} accent="#FFE6D7" /></Pressable>
    </View>
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12 }}><Ionicons name="school" size={17} color={p.brand} /><Text style={{ color: p.text, fontSize: 13, fontWeight: '700' }}>{me.data?.campusName ?? 'Campus not selected'}</Text></View>
    ...
    <View style={{ flexDirection: 'row', alignItems: 'center' }}><SectionHeader title="Your feed" /><Ionicons name={feed.isError ? 'cloud-offline' : 'cloud-done-outline'} size={20} color={feed.isError ? p.danger : p.success} style={{ marginLeft: 10, marginTop: 11 }} /></View>
