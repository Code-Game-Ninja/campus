...
const tabs = ['Posts', 'Events', 'Materials', 'Listings', 'Teams'] as const;
...
export default function MyContent() {
  ...
  const eventsQuery = useApiQuery<ApiEvent[]>(apiQueryKey('my-content', 'events'), '/events', { mine: 'true' });
  ...
  const events = useMemo(() => (eventsQuery.data ?? []).filter((event) => event.organizerId === userId).map(mapEvent), [eventsQuery.data, userId]);
  ...
  const count = tab === 'Posts' ? posts.length : tab === 'Events' ? events.length : tab === 'Materials' ? materials.length : tab === 'Listings' ? listings.length : teams.length;
  const loading = me.isLoading || (tab === 'Posts' ? postsQuery.isLoading : tab === 'Events' ? eventsQuery.isLoading : tab === 'Materials' ? materialsQuery.isLoading : tab === 'Listings' ? listingQueries.some((query) => query.isLoading) : teamQuery.isLoading);
  const error = me.error ?? (tab === 'Posts' ? postsQuery.error : tab === 'Events' ? eventsQuery.error : tab === 'Materials' ? materialsQuery.error : tab === 'Listings' ? listingQueries.find((query) => query.error)?.error : teamQuery.error);
  const refetch = () => { void me.refetch(); if (tab === 'Posts') void postsQuery.refetch(); if (tab === 'Events') void eventsQuery.refetch(); if (tab === 'Materials') void materialsQuery.refetch(); if (tab === 'Teams') void teamQuery.refetch(); if (tab === 'Listings') listingQueries.forEach((query) => void query.refetch()); };
  const create = () => router.push(tab === 'Posts' ? '/compose' : tab === 'Events' ? '/organizer/create-event' : tab === 'Materials' ? '/discover/notes/upload' : tab === 'Listings' ? '/discover/listings/new' : '/discover/tribe/new-team');
  ...
  return <Screen><TopBar title="My content" subtitle={`${count} ${tab.toLowerCase()} you can manage`} left={<IconButton icon="chevron-back" label="Back" onPress={() => goBackOrReplace('/(tabs)/profile')} />} right={<Button compact label="Create" icon="add" onPress={create} />} /><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginVertical: 12 }}>{tabs.map((item) => <Chip key={item} label={item} selected={tab === item} onPress={() => setTab(item)} />)}</View>
    {error ? <StateView icon="cloud-offline" tone="danger" title="Your content is unavailable" detail={error.message} action="Retry" onAction={refetch} /> : loading ? <StateView icon="hourglass-outline" title="Loading your content" detail="Checking ownership with the server…" /> : count === 0 ? <StateView icon="create-outline" title={`No ${tab.toLowerCase()} yet`} detail="Create content, then manage it from its detail screen." action={`Create ${tab === 'Teams' ? 'team request' : tab.slice(0, -1).toLowerCase()}`} onAction={create} /> : <>
      ...
      {tab === 'Events' ? events.map((event) => <EventCard key={event.id} event={event} />) : null}
