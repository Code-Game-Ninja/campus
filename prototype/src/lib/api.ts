  ...
  return withAuth(async (token) => {
    ...
    if (normalized === 'posts' || normalized === 'feed') {
      ...
      const items = await Promise.all(rows.map(async (post) => {
        const [labels, comments, reactions, bookmarks, rawMedia, pollState] = await Promise.all([
          visibleProfileLabels([post.author_id], token),
    ...
    if (/^posts\/[^/]+$/.test(normalized)) {
      ...
      const [labels, comments, rawMedia, pollState] = await Promise.all([
        visibleProfileLabels([rows[0].author_id], token),
    ...
    if (/^posts\/[^/]+\/comments$/.test(normalized)) {
      ...
      const labels = await visibleProfileLabels(rows.map((row) => row.author_id), token);
      const items = rows.map((row) => { const label = labels.get(row.author_id); return { id: row.id, postId: row.post_id, parentId: row.parent_comment_id, author: { userId: row.author_id, displayName: label?.displayName ?? 'Campus member', avatarUrl: label?.avatarUrl ?? null }, body: row.body, createdAt: row.created_at }; });
    ...
    if (/^team-requests\/[^/]+\/applications$/.test(normalized)) { const rows = await select<any>('team_applications', postgrestQuery({ select: '*', team_request_id: `eq.${resourceId(normalized)}`, order: 'created_at.asc' }), token); const labels = await visibleProfileLabels(rows.map((row) => row.applicant_id), token); const items = await Promise.all(rows.map(async (row) => { const teams = await select<any>('team_requests', postgrestQuery({ select: 'title', id: `eq.${row.team_request_id}` }), token); const label = labels.get(row.applicant_id); return mapApplicationRow({ ...row, applicant: label ? { display_name: label.displayName } : undefined, team: teams[0] }); })); return items as T; }
    if (normalized === 'connections') { const me = await getMe(token); const rows = await select<any>('connections', postgrestQuery({ select: '*', or: `(requester_id.eq.${me.userId},addressee_id.eq.${me.userId})`, order: 'updated_at.desc' }), token); const labels = await visibleProfileLabels(rows.map((row) => row.requester_id === me.userId ? row.addressee_id : row.requester_id), token); return rows.map((row) => { const otherId = row.requester_id === me.userId ? row.addressee_id : row.requester_id; const label = labels.get(otherId); return mapConnectionRow(row, me.userId, { display_name: label?.displayName, campus_id: label?.campusId }, me.campusId); }) as T; }
...
export async function apiRequest<T>(path: string, init: { method: 'POST' | 'PATCH' | 'PUT' | 'DELETE'; body?: unknown; idempotencyKey?: string; retry401?: boolean; headers?: Record<string, string> }): Promise<T> {
  ...
  return withAuth(async (token) => {
    ...
    if (/^team-requests\/[^/]+\/invitations$/.test(normalized) && init.method === 'POST') { const row = await rpc<any>('invite_to_team', { target_team_request_id: resourceId(normalized), target_user_id: body.targetUserId }, token); const labels = await visibleProfileLabels([row.applicant_id], token); const label = labels.get(row.applicant_id); return mapApplicationRow({ ...row, applicant: label ? { display_name: label.displayName } : undefined }) as T; }
    if (/^team-requests\/[^/]+\/applications\/[^/]+$/.test(normalized) && init.method === 'PATCH') { const row = await rpc<any>('decide_team_application', { target_application_id: finalId(normalized), decision: body.decision === 'accept' ? 'accepted' : 'rejected' }, token); const labels = await visibleProfileLabels([row.applicant_id], token); const label = labels.get(row.applicant_id); return mapApplicationRow({ ...row, applicant: label ? { display_name: label.displayName } : undefined }) as T; }
    if (/^team-requests\/[^/]+\/applications\/[^/]+$/.test(normalized) && init.method === 'DELETE') { const row = await rpc<any>('withdraw_team_application_mobile', { p_application_id: finalId(normalized) }, token); return mapApplicationRow({ ...row, applicant: { display_name: 'You' } }) as T; }
    if (/^team-requests\/[^/]+\/invitations\/[^/]+$/.test(normalized) && init.method === 'PATCH') { const row = await rpc<any>('respond_team_invitation', { target_application_id: finalId(normalized), decision: body.decision === 'accept' ? 'accepted' : 'rejected' }, token); return mapApplicationRow({ ...row, applicant: { display_name: 'You' } }) as T; }
    if (/^connections\/[^/]+$/.test(normalized) && init.method === 'PATCH') { const me = await getMe(token); const row = await rpc<any>(body.action === 'accept' || body.action === 'decline' ? 'respond_connection' : 'cancel_or_remove_connection', body.action === 'accept' || body.action === 'decline' ? { target_connection_id: resourceId(normalized), decision: body.action === 'accept' ? 'accepted' : 'declined' } : { target_connection_id: resourceId(normalized) }, token); const otherId = row.requester_id === me.userId ? row.addressee_id : row.requester_id; const labels = await visibleProfileLabels([otherId], token); const label = labels.get(otherId); return mapConnectionRow(row, me.userId, { display_name: label?.displayName, campus_id: label?.campusId }, me.campusId) as T; }
    if (normalized === 'connections' && init.method === 'POST') { const me = await getMe(token); const row = await rpc<any>('request_connection', { target_user_id: body.targetUserId }, token); const labels = await visibleProfileLabels([body.targetUserId], token); const label = labels.get(body.targetUserId); return mapConnectionRow(row, me.userId, { display_name: label?.displayName, campus_id: label?.campusId }, me.campusId) as T; }
