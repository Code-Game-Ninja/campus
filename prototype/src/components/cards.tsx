...
export function PostCard({ post, detail = false }: { post: Post; detail?: boolean }) {
  ...
  return <Card style={{ marginBottom: 14 }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}><Pressable disabled={!post.authorId} onPress={openAuthor}><Avatar initials={post.initials} accent={post.accent} /></Pressable><Pressable disabled={!post.authorId} onPress={openAuthor} style={{ flex: 1 }}><View style={{ flexDirection: 'row', gap: 6, alignItems: 'center' }}><Text style={{ color: p.ink, fontWeight: '800' }}>{post.author}</Text>{post.official ? <Ionicons name="checkmark-circle" size={15} color="#0E9384" /> : null}</View><Text style={{ color: p.muted, fontSize: 12, marginTop: 2 }}>{post.role ? `${post.role} · ` : ''}{post.time} · {post.scope === 'campus' ? 'My Campus' : 'Global'}</Text></Pressable>{isOwner ? <OwnerActions target="post" onEdit={() => router.push({ pathname: '/post/[id]', params: { id: post.id, edit: '1' } })} onDelete={async () => { try { await apiDelete(`/posts/${post.id}`); await queryClient.invalidateQueries({ queryKey: ['api', 'feed'] }); toast({ type: 'success', message: 'Post deleted.' }); } catch (error) { toast({ type: 'error', message: (error as Error).message }); } }} /> : <SafetyMenu target={post.title ?? 'post'} targetType="post" targetId={post.id} userId={post.authorType === 'person' ? post.authorId : undefined} />}</View>
    <Pressable disabled={detail} onPress={() => router.push(`/post/${post.id}`)}><View style={{ backgroundColor: post.accent, borderRadius: 14, minHeight: 112, marginTop: 14, padding: 16, justifyContent: 'flex-end' }}>{post.recruitment ? <View style={{ alignSelf: 'flex-start', marginBottom: 10 }}><Badge label="Team applications open" tone="success" icon="people" /></View> : null}{post.title ? <Text style={{ color: '#101828', fontSize: 19, lineHeight: 25, fontWeight: '900' }}>{post.title}</Text> : <Ionicons name="happy-outline" size={34} color="#344054" />}<Text style={{ color: '#344054', fontSize: 14, lineHeight: 20, marginTop: 8 }}>{post.body}</Text>{post.media?.length ? <View style={{ flexDirection: 'row', gap: 7, marginTop: 13 }}>{post.media.slice(0, 3).map((item, index) => <View key={`${item}-${index}`} style={{ flex: 1, height: 72, borderRadius: 11, backgroundColor: ['#FFFFFF99', '#DCE7FF', '#FFE0F0'][index % 3], alignItems: 'center', justifyContent: 'center' }}><Ionicons name="image-outline" size={22} color="#475467" /><Text numberOfLines={1} style={{ color: '#475467', fontSize: 10, fontWeight: '700', marginTop: 4 }}>{item}</Text></View>)}</View> : null}</View></Pressable>
    {post.recruitment && post.eventId ? <View style={{ marginTop: 10 }}><Button compact label="View roles & apply" icon="arrow-forward-circle" onPress={() => router.push(`/discover/events/${post.eventId}`)} /></View> : null}
