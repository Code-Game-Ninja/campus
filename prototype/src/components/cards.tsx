...
export function PostCard({ post, detail = false }: { post: Post; detail?: boolean }) {
  ...
  const openAuthor = () => {
    if (post.authorType !== 'person' || !post.authorId) return;
    router.push(me.data?.userId === post.authorId ? '/(tabs)/profile' : `/people/${post.authorId}`);
  };
  ...
  return <Card style={{ marginBottom: 14 }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}><Pressable disabled={!post.authorId} onPress={openAuthor}><Avatar initials={post.initials} accent={post.accent} /></Pressable><Pressable disabled={!post.authorId} onPress={openAuthor} style={{ flex: 1 }}><View style={{ flexDirection: 'row', gap: 6, alignItems: 'center' }}><Text style={{ color: p.ink, fontWeight: '800' }}>{post.author}</Text>{post.official ? <Ionicons name="checkmark-circle" size={15} color="#0E9384" /> : null}</View><Text style={{ color: p.muted, fontSize: 12, marginTop: 2 }}>{post.role ? `${post.role} · ` : ''}{post.time} · {post.campus}{post.scope === 'global' ? ' · Global' : ''}</Text></Pressable>{isOwner ? <OwnerActions target="post" onEdit={() => router.push({ pathname: '/post/[id]', params: { id: post.id, edit: '1' } })} onDelete={async () => { try { await apiDelete(`/posts/${post.id}`); await queryClient.invalidateQueries({ queryKey: ['api', 'feed'] }); toast({ type: 'success', message: 'Post deleted.' }); } catch (error) { toast({ type: 'error', message: (error as Error).message }); } }} /> : <SafetyMenu target={post.title ?? 'post'} targetType="post" targetId={post.id} userId={post.authorType === 'person' ? post.authorId : undefined} />}</View>
    <Pressable disabled={detail} onPress={() => router.push(`/post/${post.id}`)}><View style={{ backgroundColor: post.accent, borderRadius: 14, minHeight: 112, marginTop: 14, padding: 16, justifyContent: 'flex-end' }}>{post.title ? <Text style={{ color: '#101828', fontSize: 19, lineHeight: 25, fontWeight: '900' }}>{post.title}</Text> : <Ionicons name="happy-outline" size={34} color="#344054" />}<Text style={{ color: '#344054', fontSize: 14, lineHeight: 20, marginTop: 8 }}>{post.body}</Text>{mediaItems.length ? <View style={{ marginTop: 13, gap: 7 }}>{mediaItems[0].type === 'image' ? <Image source={{ uri: mediaItems[0].url }} style={{ width: '100%', aspectRatio: 4 / 5, borderRadius: 11, backgroundColor: '#FFFFFF99' }} resizeMode="cover" /> : <Pressable onPress={() => void Linking.openURL(mediaItems[0].url)} style={{ minHeight: 82, borderRadius: 11, backgroundColor: '#FFFFFF99', alignItems: 'center', justifyContent: 'center', padding: 8 }}><Ionicons name="document-text-outline" size={24} color="#475467" /><Text numberOfLines={2} style={{ color: '#475467', fontSize: 10, fontWeight: '700', marginTop: 4, textAlign: 'center' }}>{mediaItems[0].name ?? 'Document'}</Text></Pressable>}{mediaItems.length > 1 ? <View style={{ flexDirection: 'row', gap: 7 }}>{mediaItems.slice(1, 3).map((item, index) => item.type === 'image' ? <Image key={`${item.url}-${index}`} source={{ uri: item.url }} style={{ flex: 1, aspectRatio: 1, borderRadius: 9, backgroundColor: '#FFFFFF99' }} resizeMode="cover" /> : <View key={`${item.url}-${index}`} style={{ flex: 1, aspectRatio: 1, borderRadius: 9, backgroundColor: '#FFFFFF99', alignItems: 'center', justifyContent: 'center' }}><Ionicons name="document-text-outline" size={20} color="#475467" /></View>)}</View> : null}</View> : null}</View></Pressable>
